	...
	public String login() {
		...
		if (existUser != null) {
			...
			if ("0".equals(existUser.getUser_status())) {
				...
				// 用户账号为冻结状态登陆失败
				// 设置回显信息
				this.addActionError("此用户账号已被冻结，请联系客服了解详情。");
				return LOGIN;
			}
		} else {
			// 登陆失败
			// 设置错误信息
			this.addActionError("找不到该用户：请确认用户账号密码。");
			return LOGIN;
		}
...
//			String url = path + realPath;
	...
	public String editUser() throws IOException {
		...
		System.out.println("ssssssss");
		user.setUser_headimge(users.getUser_headimge());
		// 上传图片:
		if (upload != null) {
			System.out.println("ssssssss");
...
//				}
	...
	public String editStatus() {
		System.out.println("SSSSSSSSSSS"+user.getUser_id());
		User findById = userService.findById(user.getUser_id());
		if("1".equals(findById.getUser_status())) {
			System.out.println("SSSSSSSSSSS");
			findById.setUser_status("0");
			userService.editUser(findById);
		}else {
			System.out.println("SSSASSSSSSSS");
			findById.setUser_status("1");
			userService.editUser(findById);
		}
		return NONE;
	}
