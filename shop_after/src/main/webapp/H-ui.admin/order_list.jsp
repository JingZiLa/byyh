...
<body>
	...
	<div class="page-container">
		<form
			action="${ pageContext.request.contextPath }/order_findAll.action"
			method="post">
			<div class="text-c">
				订单ID： <input type="text" name="order_number" id=""
					placeholder=" 请输入订单ID：" style="width: 250px" class="input-text"
					value="<s:property value="order_number"/>"> 订单状态:
					<select style="height:30px" name="findStatus">
						<s:if test="findStatus != '' && findStatus != null">
							<s:property value="findStatus"/>
						</s:if>
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a href="javascript:;" onclick="datadel()"
					class="btn btn-danger radius"> <i class="Hui-iconfont">&#xe6e2;</i>
						批量删除
				</a></span> <a href="${ pageContext.request.contextPath }/order_createUI.action" 
					class="btn btn-success radius"> 
						创建订单（测试）
				</a><span class="r">共有订单<strong><s:property
						...
						<tr class="text-c">
							<s:iterator value="list">
								<tr class="text-c">
									...
									<td><s:date name="order_addtime"
											format="yyyy-MM-dd hh:MM:ss" />
										&nbsp;&nbsp;&nbsp;&nbsp;订单ID:<s:property value="order_number" />
										<br /> <s:property value="mdse_name" /> <s:property
											value="mdse_mome" /></td>

									<td><s:property value="" />10</td>
									<td><s:property value="mdse_name" />￥：100</td>
									<td>用户ID:<s:property value="user.user_id" /> <br /> <s:property
											value="user.user_name" />
									</td>
									<td>用户ID:<s:property value="user.user_id" /> <br /> <s:property
											value="user.user_name" />
									</td>
									<td class="td-status">付款状态:
									<s:if test="order_status == 0 ">
									...
									<a style="text-decoration: none" class="ml-5"
										onClick="status_stop(this,<s:property value="mdse_id" />)"
										href="${ pageContext.request.contextPath }/order_editOrder.action?order_id=<s:property value="order_id" />&id=1"
										title="修改"><i class="Hui-iconfont">&#xe6df;</i></a> 
									<br /> 发货状态: 
									<s:if test="order_cargo_status == 0 ">
										<span class="label label-defaunt radius" style="margin: 5px 0;"> 待发货
										</span> 
									
									</s:if>
									<s:if test="order_cargo_status == 1 ">
									 ...
									 <a style="text-decoration: none" class="ml-5"
										onClick="status_stop(this,<s:property value="mdse_id" />)"
										href="${ pageContext.request.contextPath }/order_editOrder.action?order_id=<s:property value="order_id" />&id=2"
										title="修改"><i class="Hui-iconfont">&#xe6df;</i></a>
									<br /> 收货状态: 
									<s:if test="order_pay_status == 0 ">
										<span class="label label-defaunt radius"
										style="margin: 5px 0;">
										 待收货
									</span>
									</s:if>
									<s:if test="order_pay_status == 1 ">
									 ...
									 <a style="text-decoration: none" class="ml-5"
										onClick="status_stop(this,<s:property value="mdse_id" />)"
										href="${ pageContext.request.contextPath }/order_editOrder.action?order_id=<s:property value="order_id" />&id=3"
										title="修改"><i class="Hui-iconfont">&#xe6df;</i></a>
									</td>
									<td class="td-manage">
										<a href="${ pageContext.request.contextPath }/order_orderDetails.action?order_id=<s:property value="order_id" />" style="margin-left: -10%;">
