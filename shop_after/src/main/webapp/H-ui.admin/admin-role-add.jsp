...
<article class="page-container">
	<form action="<%=request.getContextPath() %>/admin_add.do" method="post" class="form form-horizontal">
			<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="adminAs" size="1">
				<option value="拥有至高无上的权利">拥有至高无上的权利</option>
				<option value=" 具有添加、审核、发布、删除内容的权限 "> 具有添加、审核、发布、删除内容的权限 </option>
				<option value="只对所在栏目具有添加、审核、发布、删除内容的权限">只对所在栏目具有添加、审核、发布、删除内容的权限</option>
				<option value="只对所在栏目具有添加、删除草稿等权利">只对所在栏目具有添加、删除草稿等权利</option>
			</select>
			</span> </div>
	</div>
		</hr>
				...
				<label>用户名:</label>
		  		<select class="form-control input-gr" name="storeid" style="margin-left:30px">
		  			<c:forEach items="${list}" var="store">
		  				<option value="${store.id }" style="height: 20px">${store.username }</option>
		  			</c:forEach>
		  		</select>
		</div>
