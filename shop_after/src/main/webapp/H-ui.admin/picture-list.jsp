...
<form action="<%=request.getContextPath() %>/img_select.do" method="post">
	<div class="text-c"> 日期范围：
		<input type="text" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}' })" id="logmin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:120px;">
		<input type="text" name="select" id="" placeholder=" 图片名称" style="width:250px" class="input-text">
	...
	<a class="btn btn-primary radius" href="H-ui.admin/picture-add.jsp">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<tr class="text-c">
			<c:forEach items="${list}" var="store" varStatus="st">
				<tr class="text-c">
					<td><input name="" type="checkbox" value=""></td>
					<td>${store.id}</td>
					<td>${store.articlecolumn}</td>
					<td><a href="javascript:;" onClick="picture_edit('图库编辑','picture-show.html','10001')"><img width="210" class="picture-thumb" src="<%=request.getContextPath() %>/upload/${store.img}"></a></td>
					<td class="text-l"><a class="maincolor" href="javascript:;" onClick="picture_edit('图库编辑','picture-show.html','10001')">${store.imgname}</a></td>
					<td class="text-c">${store.label }</td>
					<td>${store.time }</td>
					...
					<a style="text-decoration:none" class="ml-5" onClick="picture_edit('图库编辑','picture-add.jsp','10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
					<a style="text-decoration:none" class="ml-5"  href="img_delect.do?id=${store.id }" title="删除" >
...
<script type="text/javascript" src="<%=request.getContextPath() %>/H-ui.admin/lib/datatables/1.10.15/jquery.dataTables.min.js"></script> 
<script type="text/javascript" src="<%=request.getContextPath() %>/H-ui.admin/lib/laypage/1.2/laypage.js"></script>
